<template>
	...
	<el-dialog :model-value="VisibleAdd" :close-on-click-modal="false" width="70%" @close="VisibleAdd=false"
		style="border-radius: 10px;height: 70%;">
		<el-scrollbar height="5rem">
...
<script setup>
	...
	const emit = defineEmits(['goOut', 'complete',"Editcomplete"])
	// 取消按钮
	const goOut = () => {
		emit('goOut')
	}
	...
	const determineAdd = () => {
		...
		form.value.optiop = []
		type.value[index-1]=radioAdd.value+1
		// type.value.splice(index, 0, radioAdd.value);
		// console.log(type.value);
